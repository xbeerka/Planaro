...
- **Visual Regression (Inner Corners during Resize)**:
  - ✅ **Проблема**: При ресайзе склеенного события внутренние углы исчезали до завершения операции.
  - ✅ **Причина**: CSS правило `.scheduler-event.resizing.inner-tl::before { display: none !important }` скрывало псевдоэлементы.
  - ✅ **Решение**: Удалены правила скрытия внутренних углов при `.resizing`.
  - ✅ **Результат**: Внутренние углы остаются видимыми во время ресайза, визуальная склейка не ломается.
...
- **Решение**:
  ...
  - ✅ **Умное логирование с счетчиком ошибок**:
    ...
    - При успешной отправке ��четчик сбрас��вается
