...
**Новый таймлайн** (защита от гонки условий):
```
t=0ms    : Resize завершён, setIsUserInteracting(false)
...
### 🐛 FIX: Удаление событий синхронизируется между пользователями (2025-11-18)
...
**Исправлен критический баг в алгоритме склейки событий - поджатие справа теперь работает для коротких событий**
...
- ❌ В середине горизонтальной склейки события **без боковых отступов вообще**
...
10. ✅ handleCellMouseLeave → НЕ убирает hover если меню открыто (правильная логика)
...
- ✅ **Небольшая задержка** (10ms) между сотрудниками для визуального обновления
...
- Обновлён: `/components/scheduler/SchedulerMain.tsx` - интеграция компонента
...
#### Визуальный стиль
- SVG стрелка курсора с уникальным цветом для каждого пользователя
...
- **Правил��ная логика отображения онлайн пользователей**:
  ...
  - **Обновление токена после изменения профиля**:
    ...
    - Это необходимо чтобы получить свежий токен с обновлёнными user_metadata (displayName, avatarUrl)
  ...
  - Исправлен batch endpoint response parsing: `data.workspaces` → `data` (сервер возвращает results напрямую)
...
- **Логика отображения текущего пользователя в Presence системе**:
  ...
  - Внутри календаря (OnlineUsers) - текущий пользователь ПОКАЗЫВАЕТСЯ (чтобы было понятно "кто я")
  - Убрана золотая рамка для текущего пользователя
  - Метка "(вы)" теперь зеленого цвета вместо желтого
  - Добавлены детальные debug логи для диагностики проблем с онлайн пользователями
