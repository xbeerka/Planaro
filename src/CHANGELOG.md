...
**Одновременное изменение двух соседних событий через промежуток между ними**
...
**Решение**: Создана заглушка в `/utils/supabase/client.ts` - всегда возвращает `null`
...
- Сделал **Undo** → событие восстановилось с **прежней** высотой
...
1. **Заменили state на ref**:
   ...
   // Стало: const hasCachedDataRef = useRef(false);
   ```
...
2. **Усилена защита в `saveHistory`** (useHistory.ts):
   - Если есть события но НЕТ проектов → НЕ сохраняем в историю (return early)
...
- ✅ Синхронизация всех операций с сервером:
  ...
  - Undo изменения → UPDATE запрос к БД
  - Redo → аналогично
...
- `"✅ Событие восстановлено с новым ID: e456 → e789"`
...
**Исправлен баг в алгоритме вклинивания - откусывание теперь срабатывает только для событий с двойным расширением**
...
- ❌ При `hasChanged = false` код делал `return`, но оставлял изменённые стили
...
**Исправлены ошибки подключения WebSocket для collaborative cursors**
...
- **Проблема**: Пользователи оставались "онлайн" 2+ минуты после закрытия календаря
  ...
  - Результат: в списке воркспейсов показывались "призраки" (ушедшие пользователи)
...
- **Решение - трёхступенчатое**:
  1. **Явный "leave" при размонтировании OnlineUsers**:
     ...
     - Вызывается в `useEffect` cleanup при закрытии календаря
...
- **Мгновенное отображение онлайн пользователей в календаре**:
  - **Проблема**: При входе в календарь OnlineUsers делал запрос к `/presence/online/:workspaceId` → задержка 200-500ms
  ...
  - **Детальное логирование**: 
    ...
    - `💾 OnlineUsers: загружено из кэша (Nс назад): M пользователей`
...
- **URL роутинг**: Окончательно исправлена ошибка "Воркспейс не найден" при навигации
  ...
  - Программная навигация (клик по воркспейсу) → устанавливает флаг → пропускает восстановление
