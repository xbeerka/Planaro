...
- **IndexedDB** для х��анения access_token и session_id на клиенте
...
/supabase/functions/server
  ...
  - kv_store.tsx - утилиты для KV т��блицы (НЕ ИЗМЕНЯТЬ)
```
...
- **Presence система** - отслеживание онлайн пользователей:
  ...
  - **Отображение внутри календаря** (OnlineUsers):
    - **КРИТИЧНО**: Текущий пол��зователь ВСЕГДА берется ИЗ ТОКЕНА (не из presence!)
...
- **Оптимизации**:
  ...
  - Блокировка синхронизации при взаимодействии (нет конфликтов)
  - Пропуск синхронизации после локальных изменений (нет "мигания")
