...
**Версия документа**: 4.0.3 (2025-11-28)
**Статус**: STABLE (Gluing v6.18 + Undo Fix)
**Последнее обновление**: 
- **Event Gluing Logic (v6.18)**:
  - ✅ **Undo/Redo Fix**: Исправлен порядок расчета скрытия названий (сортировка по времени)
  - ✅ **Rule Based Logic**: Внедрены правила для обработки краевых сценариев (A/B/C/D формы)
  - ✅ **Stacking Reduction**: Уменьшение отступов для "Notch" событий
  - ✅ **Boost Squeezed**: Расширение соседей при "поджатии" основного события
  - ✅ **Clean Pass 2**: Убрана блокировка расширения для Walls
- **Fixed React Warning**: "Cannot update component while rendering"
- **Gap Handles (Group Size) Fixes**:
  - ✅ **Race Condition Fix**: Исправлена проблема "отката" изменений при быстром последовательном использовании Gap Handles.
  - ✅ **flushSync**: Использование `flushSync` гарантирует мгновенное обновление стейта перед следующим взаимодействием.
  - ✅ **EventsRef**: Использование `eventsRef` в `useGapInteractions` для доступа к всегда актуальным данным (защита от stale closures).
  - ✅ **Consistent State**: Расчет изменений теперь всегда базируется на `initialAllEvents` снапшоте.
  - ✅ **Validation**: Добавлена строгая валидация для attached событий (нельзя сжать меньше 1 недели).
- **SyncManager Improvements**:
  - ✅ **Smarter Flush**: `flush()` теперь не просто пропускает вызов если занят, а пла��ирует повторную попытку (reschedule).
  - ✅ **Toast Errors**: Добавлено отображение ошибок (Toast) при частичных сбоях batch операций.
