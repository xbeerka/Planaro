...
import { ProjectAnalyticsModal } from "../components/scheduler/ProjectInfoPopover";
...
function Frame4(props: HeaderProps) {
  ...
  return (
    <div className="content-stretch flex gap-[8px] items-center relative shrink-0 max-md:hidden">
      {/* Navigation buttons group - hidden for viewers */}
      {!isViewer && (
        <>
      ...
      {/* Tool buttons - cursor always visible, scissors hidden for viewers */}
      <ModeButton
        active={isCursor}
...
function Input6({ sidebarCollapsed }: HeaderProps) {
  ...
  useEffect(() => {
    ...
    if (isOpen) {
      // capture: true — ловим событие ДО stopPropagation() в scheduler events
          ...
          >
            ...
            <div className="overflow-y-auto flex-1 px-4 py-4 space-y-6">
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center justify-between">
                  <span className="text-sm text-gray-900">
                    Цветные департаменты
                  </span>
                  <Switch
                    checked={showDeptColors}
                    onCheckedChange={setShowDeptColors}
                  />
                </div>
...
function Frame2(props: HeaderProps) {
  ...
  return (
    <div className="content-stretch flex gap-[8px] items-center relative shrink-0">
      ...
      {!sidebarCollapsed && (
        <>
          {/* Separator between Фильтр+Вид and Online Users */}
          <div className="w-[1px] h-6 bg-gray-300 shrink-0 max-md:hidden" />
...
function Container(props: HeaderProps) {
  ...
  const { events, resources, grades, companies: schedulerCompanies } = useScheduler();
  ...
  // ✨ Состояние для модалки аналитики проекта
  ...
  const handleClearAll = (e: React.MouseEvent) => {
    ...
    setSearchQuery("");
    console.log(`🧹 Фильтр "${activeTab}" очищен`);
  };
  ...
  // Получение количества фильтров для текущей вкладки
  const currentTabFilterCount = activeTab === "company" ? enabledCompanies.size
    : activeTab === "department" ? enabledDepartments.size
    : enabledProjects.size;
  ...
  return (
    <div ref={containerRef} className="relative flex items-center gap-2">
      {/* Кнопка "Сводка" — слева от фильтра */}
      <div
      ...
      >
        <div
          aria-hidden="true"
          className={`absolute border-[0.8px] ${analyticsOpen ? "border-[#0062FF]" : "border-[rgba(0,0,0,0.12)]"} border-solid inset-0 pointer-events-none rounded-[12px]`}
        />
        <div className="relative shrink-0 size-[20px] flex items-center justify-center">
          <BarChart3
            className={`w-[16px] h-[16px] ${analyticsOpen ? "text-[#0062FF]" : "text-black"}`}
          />
        </div>
        <p
          className={`font-medium leading-[20px] relative shrink-0 text-[12px] ${analyticsOpen ? "text-[#0062FF]" : "text-black"} text-nowrap whitespace-pre`}
        >
          Сводка
        </p>
      </div>

      {/* Separator between Сводка and Фильтр */}
      <div className="w-[1px] h-6 bg-gray-300 shrink-0" />
      ...
      {isOpen &&
        createPortal(
          ...
          >
            ...
            {/* Заголовок */}
            <div className="px-4 border-b border-gray-200 h-[52px] shrink-0 flex items-center justify-between">
              <h3 className="font-semibold text-base max-md:text-[17px]">Фильтр</h3>
              <div className="flex items-center gap-2">
                {totalFilters > 0 && (
                  <span className="text-xs text-gray-500">
                    {totalFilters} {totalFilters === 1 ? 'фильтр' : totalFilters < 5 ? 'фильтра' : 'фильтров'}
                  </span>
                )}
            ...
            {/* Поиск + Текущая неделя + Сбросить — ПОД вкладками, одна строка */}
            <div className="px-4 py-2.5 border-b border-gray-200 shrink-0 flex items-center gap-2">
              ...
              {/* «Текущая неделя» — компактный чип, только для вкладки Проекты с выбранными */}
              {activeTab === "project" && enabledProjects.size > 0 && (
                <button
                  type="button"
                  onClick={toggleProjectFilterTodayOnly}
                  className={`inline-flex items-center gap-1.5 h-9 px-3 rounded-lg text-xs font-medium transition-colors cursor-pointer shrink-0 whitespace-nowrap border ${
                    projectFilterTodayOnly
                      ? "bg-[#0062FF] text-white border-[#0062FF]"
                      : "bg-white text-gray-600 border-gray-300 hover:bg-gray-50"
                  }`}
                  title="Показать только проекты на текущей неделе"
                >
              ...
              {currentTabFilterCount > 0 && (
                <button
                  onClick={handleClearAll}
                  className="flex items-center justify-center h-9 w-9 shrink-0 rounded-lg border border-gray-300 text-gray-400 hover:text-red-500 hover:border-red-300 hover:bg-red-50 transition-colors cursor-pointer"
                  aria-label="Сбросить фильтр текущей вкладки"
                  title="Сбросить"
                >
                  <X className="w-4 h-4" />
                </button>
                ...
                filteredItems
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* Removed info button - analytics is now a separate button in toolbar */}
                    </div>
                  );
                })
              )}
      ...
      {analyticsOpen && (
        <ProjectAnalyticsModal
          ...
          grades={grades}
          companies={schedulerCompanies}
          projects={projects as any[]}
          onClose={() => setAnalyticsOpen(false)}
          offWeekNumbers={props.offWeekNumbers}
        />
      )}
    </div>
  );
