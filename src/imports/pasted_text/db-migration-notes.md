Ниже — текст в формате .md, уже пригодный для передачи в Figma Make. Я сделал его в логике: что меняется, какая сейчас структура у таблиц, что означает каждая таблица.
Если в проекте ещё остались старые имена полей/таблиц, Figma Make сможет по этому описанию привести всё к новой модели.
