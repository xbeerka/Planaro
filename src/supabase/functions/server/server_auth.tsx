...
export function registerAuthRoutes(app: Hono) {
    ...
    try {
      ...
      if (error) {
        ...
        if (error.message.includes('Email not confirmed')) {
          console.log('📨 Вход: Email не подтвержден. Отправка OTP...');
          
          // Пытаемся отправить OTP
  ...
  app.post("/make-server-73d66528/auth/verify-otp", async (c) => {
    try {
      ...
      console.log('✅ OTP подтвержден, вход выполнен:', data.user.email);
  ...
  app.post("/make-server-73d66528/auth/signup", async (c) => {
    try {
      ...
      // ВАЖНО: Мы используем email_confirm: true, чтобы автоматически подтверждать почту @kode.ru.
      // Это позволяет избежать блокировок при регистрации и проблем с доставкой писем.
      ...
      // Логиним пользователя сразу после создания
      ...
      if (signInError) {
        console.error('⚠️ Ошибка автоматического входа:', signInError.message);
        
        // Если ошибка "Email not confirmed" - значит настройки сервера требуют подтверждения
        if (signInError.message.includes('Email not confirmed')) {
          console.log('📨 Требуется подтверждение email. Письмо должно быть отправлено Supabase автоматически.');
          
          // ВАЖНО: Мы НЕ отправляем signInWithOtp повторно здесь, чтобы избежать ошибки "email rate limit exceeded".
          // admin.createUser (с email_confirm: false) уже инициировал отправку письма.
          
          return c.json({ 
            success: true, 
            requiresOTP: true,
            message: 'Email не подтвержден. Код или ссылка отправлены на вашу почту.',
            user_id: data.user.id
          }, 200);
        }
      ...
      const sessionExpiry = Date.now() + 30 * 24 * 60 * 60 * 1000; // 30 дней
