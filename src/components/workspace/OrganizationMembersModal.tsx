...
import { organizationMembersApi, OrgAccessEntry, OrgInfo } from '../../services/api/organizationMembers';
...
export function OrganizationMembersModal({ onClose, accessToken, orgId, orgName }: OrganizationMembersModalProps) {
  ...
  // Realtime: подписка на изменения organization_members и organization_invites
  useEffect(() => {
    if (!orgId) return;
    const oid = String(orgId);
    console.log(`🔌 OrgMembersModal: подписка на Realtime для org ${oid}`);

    const channel = supabaseClient
      .channel(`share-org-${oid}`)
      .on('postgres_changes', {
        event: '*',
        schema: 'public',
        ...
        console.log('🔌 OrgMembersModal Realtime: organization_members changed', payload.eventType);
        loadData();
      })
      .on('postgres_changes', {
        event: '*',
        ...
        console.log('🔌 OrgMembersModal Realtime: organization_invites changed', payload.eventType);
        loadData();
      })
      .subscribe((status) => {
        console.log(`🔌 OrgMembersModal Realtime status: ${status}`);
      });

    return () => {
      console.log(`🔌 OrgMembersModal: отписка от Realtime для org ${oid}`);
    ...
    try {
      ...
      if (orgId) {
        const [orgInfo, members] = await Promise.all([
          organizationMembersApi.getOrgById(orgId, accessToken || undefined),
          organizationMembersApi.getMembers(orgId, accessToken || undefined),
        ]);
        org = orgInfo;
        setOrgInfo(org);
        setEntries(members);
        console.log(`🏢 Организация: ${org.name} (id: ${org.id}), ${members.length} участников`);
      } else {
        org = await organizationMembersApi.getMyOrganization(accessToken || undefined);
        setOrgInfo(org);
        console.log(`🏢 Организация: ${org.name} (id: ${org.id})`);

        const data = await organizationMembersApi.getMembers(org.id, accessToken || undefined);
        setEntries(data);
        console.log(`👥 Загружено ${data.length} участников организации`);
      }
    } catch (err: any) {
  ...
  const handleInvite = async () => {
    ...
    const existingEmails = new Set(entries.map(e => e.email.toLowerCase()));
    ...
    setIsInviting(true);
    try {
      const result = await organizationMembersApi.inviteMembers(
        orgInfo.id, [email], selectedRole, accessToken || undefined
      );
      ...
      toast.success(parts.length > 0 ? parts.join(', ') : 'Приглашение отправлено', {
        description: result.pending_emails.length > 0
          ? `${result.pending_emails.join(', ')} — ожидают регистрации`
          : 'Пользователь получит уведомление'
      });

      setEmailInput('');
      setSelectedRole('viewer');
      const data = await organizationMembersApi.getMembers(orgInfo.id, accessToken || undefined);
      setEntries(data);
    } catch (err: any) {
  ...
  const handleUpdateRole = async (entry: OrgAccessEntry, newRole: 'editor' | 'viewer') => {
    if (!orgInfo || entry.role === newRole) { setOpenDropdownId(null); return; }
    ...
    try {
      if (entry.type === 'member' && entry.user_id) {
        await organizationMembersApi.updateMemberRole(orgInfo.id, entry.user_id, newRole, accessToken || undefined);
      } else if (entry.type === 'invite' && entry.invite_id) {
        await organizationMembersApi.updateInviteRole(orgInfo.id, entry.invite_id, newRole, accessToken || undefined);
      }
  ...
  const handleRemove = async (entry: OrgAccessEntry) => {
    if (!orgInfo) return;
    ...
    try {
      if (entry.type === 'member' && entry.user_id) {
        await organizationMembersApi.removeMember(orgInfo.id, entry.user_id, accessToken || undefined);
      } else if (entry.type === 'invite' && entry.invite_id) {
        await organizationMembersApi.revokeInvite(orgInfo.id, entry.invite_id, accessToken || undefined);
      }
      toast.success(
  ...
  const getEntryKey = (entry: OrgAccessEntry): string => {
        ...
        <div className="p-[20px]">
          <div className="flex items-start justify-between mb-[16px]">
            <div className="flex flex-col gap-[2px]">
              ...
              <p className="text-[12px] text-[#868789] leading-normal">Организация «{orgInfo?.name || orgName || '...'}»</p>
            </div>
            <button
              onClick={onClose}
          ...
          <div className="flex gap-[8px] h-[36px] items-center">
            ...
            <div className="flex-1 h-[36px] bg-[#f6f6f6] rounded-[10px] flex items-center">
              <input
                ...
                className="flex-1 h-full pl-[12px] pr-[4px] text-[13px] bg-transparent border-none outline-none placeholder:text-[#999] text-[#333]"
                style={{ minWidth: 0 }}
              />
            ...
            <button
              ...
              disabled={isInviting || !hasEmailInput}
...
interface OrgMemberRowProps {
  ...
  onUpdateRole?: (entry: OrgAccessEntry, role: 'editor' | 'viewer') => void;
  onRemove?: (entry: OrgAccessEntry) => void;
...
function OrgMemberRow({
  ...
  onUpdateRole, onRemove, currentUserId,
}: OrgMemberRowProps) {
    ...
    <div className="flex items-center justify-between py-[7px]">
      ...
      <div className="flex flex-1 gap-[8px] items-center min-w-0">
        ...
        <div className="flex flex-col min-w-0">
          {isInvite ? (
            /* Pending: show email + (Отправлено), single line, no second row */
            ...
            <>
              <p className="text-[12px] text-[#1a1a1a] leading-normal truncate">
                ...
                {isCurrentUser && <span className="text-[#999]"> (Вы)</span>}
              </p>
      ...
      {isOwner ? (
        ...
        <div className="relative shrink-0" data-role-dropdown>
          <button
            ref={buttonRef}
            onClick={() => setOpenDropdownId?.(isDropdownOpen ? null : (entryKey || null))}
            className="flex items-center gap-[4px] text-[12px] text-[#999] leading-normal whitespace-nowrap transition-colors hover:text-[#666]"
          >
            {roleLabel}
            <svg width="12" height="12" viewBox="0 0 12 12" fill="none" style={{ opacity: 0.5 }}>
