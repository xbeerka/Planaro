...
export const ProjectsManagementContent = forwardRef<ProjectsManagementHandle, ProjectsManagementContentProps>(({
  ...
  useEffect(() => {
    ...
    onHasChanges(hasNewProjects || hasDeletedProjects || hasExistingChanges);
  }, [localNewProjects, deletedProjectIds, editingProjects, projects, onHasChanges]);
  ...
  const generateRandomColors = () => {
    ...
    const backgroundColor = `#${toHex(r)}${toHex(g)}${toHex(b)}`.toUpperCase();
    const textColor = '#FFFFFF'; // Белый текст для ярких цветов
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editingProjects && typeof editingProjects === 'object') {
        for (const projectId in editingProjects) {
          if (deletedProjectIds.includes(projectId)) continue;

          const editedData = editingProjects[projectId];
          if (!editedData || typeof editedData !== 'object') {
            console.warn(`⚠️ Пропуск проекта ${projectId}: editedData невалидна`, editedData);
            continue;
          }
        ...
        console.warn('⚠️ editingProjects невалидный объект:', editingProjects);
      }
  ...
  const renderPatternPreview = (backgroundColor: string, patternId: string, textColor: string, showText: boolean = true) => {
    const style: React.CSSProperties = {
      backgroundColor: backgroundColor?.trim() ? backgroundColor : '#e5e5e5',
      cursor: 'pointer'
    };
    
    if (patternId) {
      const projectPattern = eventPatterns.find(p => p.id === patternId);
  ...
  const visibleProjects = useMemo(() => {
    let filtered = projects.filter(p => !deletedProjectIds.includes(p.id));
    ...
    return filtered;
  }, [projects, deletedProjectIds, searchQuery]);
