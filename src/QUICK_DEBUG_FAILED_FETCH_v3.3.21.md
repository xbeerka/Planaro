# Быстрая диагностика "Failed to fetch" v3.3.21
...
#### ✅ ХОРОШО - Видны все параметры:
```
📦 BATCH: всего операций для отправки: 3
📦 BATCH: 2 create + 1 update
📦 BATCH: Отправка запроса к: https://YOUR_PROJECT_ID.supabase.co/functions/v1/make-server-73d66528/events/batch
📦 BATCH: Authorization: Bearer eyJhbGciOiJIUzI1NiIsI...
📦 BATCH: Workspace ID: 14
❌ Failed to fetch (попытка 1/3)
```
**→ Сервер недоступен. Переходите к пункту 2.**

#### ❌ ПЛОХО - Ошибка ДО проверки:
```
❌ BATCH: accessToken не определён!
Error: Access token is required for batch operations
```
**→ Проблема с авторизацией. Переходите к пункту 3.**

#### ❌ ПЛОХО - Ошибка ДО проверки:
```
❌ BATCH: projectId не определён!
Error: Project ID is required for batch operations
```
...
- [ ] В консоли видны логи `📦 BATCH: Отправка запроса...`
